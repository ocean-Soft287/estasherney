...
class AppointmentFutureCubit  extends Cubit<ApointmentFutureState>{
  ...
  AppointmentFutureCubit(this.doctorFutureAppointmentRepo) : super(AppointmentFutureInitial()) {
    getFuturedoctorAppointments();
  }

  Future<void> getFuturedoctorAppointments() async {
    emit(AppointmentFutureLoading());
    final result = await doctorFutureAppointmentRepo.getFuturedoctorAppointments();
    result.fold(
      (failure) => emit(AppointmentFutureFailure(failure.message)),
      (appointments) => emit(AppointmentFutureSuccess(appointments)),
    );
  }
