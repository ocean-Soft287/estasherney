...
class PaymentPageState extends State<PaymentPage>
  ...
  final Map<int, Map<String, dynamic>> _paymentMethods = {
    ...
    11: {"name": "أومان نت", "icon": Icons.account_balance, "color": Colors.orange},
  };
  ...
  void initiatePayment() async {
   ...
   sourceCurrency: 'EGP', destinationCurrency: 'KWD');
    final request = MFInitiatePaymentRequest(
      invoiceAmount:myPriceInUSD,
      currencyIso: 'USD',
    );

   MFInitiatePaymentResponse res =  await MFSDK.initiatePayment(request, MFLanguage.ARABIC);
  ...
  double myPriceInUSD = await fx.getCurrencyConverted(sourceAmount: widget.response.finalConsultationPrice,
   sourceCurrency: 'EGP', destinationCurrency: 'KWD');
    ...
    try {
    final res  =   await MFSDK.executePayment(request, MFLanguage.ARABIC, (invoiceId) {
